 ...
 * @author OussKh
 */
public class AfficherDemandeForm extends BaseForm {

    Form current;
    Resources res;

    public AfficherDemandeForm(Form previous ) {
...
//        sl.setBackgroundType(Style.BACKGROUND_IMAGE_SCALED_FILL);
        ...
        SpanLabel labeldesc = new SpanLabel("nomterrain");
        SpanLabel labelde = new SpanLabel("nomequipe");
...
//        HeadConainter.add(labelup);
        ...
        ArrayList<Demande> themes = ServiceDemande.getInstance().getAllDemande();
        for (Demande d : themes) {
            //delete button
            ...
            lSupprimer.addPointerPressedListener(l -> {
                ...
                if (dig.show("Supprimer", "aimeriez-vous supprimer demande? cette action ne sera pas annulée !", "Cancel", "OK")) {
                    dig.dispose();
                } else {
                    dig.dispose();
                    // appel de la fonction delete du service Experience
                    if (ServiceDemande.getInstance().DeleteDemande(d)) {
            ...
            lModifier.addPointerPressedListener(l -> {
                new ModifierDemandeForm(d).show();
            });

            Container BodyConainter = new Container(gridLayout);
            BodyConainter.add(new Label(d.getDate()));
            BodyConainter.add(new Label(d.getNomterrain()));
            BodyConainter.add(new Label(d.getNomequipe()));
...
//                                || line4 != null && line4.toLowerCase().contains(text);
        ...
        Addnew.addActionListener(e -> new AjouterDemandeForm(current).show());
        tempForm.add(Addnew);
        tempForm.show();
