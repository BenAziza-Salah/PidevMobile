...
public class ListCatsForm  extends Form {
 ...
 Toolbar tb = new Toolbar(true);
        ...
        Label labelnomcat = new Label("nomcat");
        ...
        labelnomcat.getUnselectedStyle().setFont(fnt);
...
//                 e -> new HomeReview());
        ...
        ArrayList<Categoriearticle> Categorie = ServiceCat.getInstance().getAllcats();
        for (Categoriearticle r : Categorie) {
         //  if (r.getNom_client_review().equals(Username)) {
                Container BodyConainter = new Container(gridLayout);
                BodyConainter.add(new Label(r.getNomcat()));
    ...
    if (ServiceCat.getInstance().deletecat(r)) {                           
        Dialog.show("Success", "Category has been deleted! ", "Ok", null);
//                            Form NewForm = ListCatsForm();
//                            NewForm.setTransitionOutAnimator(CommonTransitions.createEmpty());
//                            NewForm.show();
                        } else {
                            Dialog.show("ERROR", "Error , Could NOT delete Category", "Ok", null);
                        }
                    }
        ...
        Button AddCategory = new Button("Add a Category");
        AddCategory.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent evt) {
                tempForm.setTransitionOutAnimator(CommonTransitions.createSlide(CommonTransitions.SLIDE_HORIZONTAL, false, 250));
                new AddCatForm(tempForm).show();
                tempForm.setTransitionOutAnimator(CommonTransitions.createEmpty());
            }
        });
